...
const sampleProducts = [
  {id:1,name:'Ebook: Sustainable Living',price:350,desc:'Guide to eco living',cat:'ebooks'},
  {id:2,name:'Website Template: Green Shop',price:799,desc:'HTML/CSS template',cat:'templates'},
  {id:3,name:'UI Kit Pack',price:499,desc:'Icons & components',cat:'templates'},
  {id:4,name:'Course: Composting',price:120,desc:'Video course',cat:'ebooks'},
  {id:5,name:'Photos Pack',price:250,desc:'Stock photos',cat:'templates'},
  {id:6,name:'Plugin: Analytics',price:1299,desc:'Downloadable plugin',cat:'templates'}
];
...
function renderFeatured(){
  ...
  sampleProducts.slice(0,4).forEach(p=>{
    ...
    col.innerHTML = `<div class="card-e p-3 h-100"><div class="product-thumb">📦</div><h5 class="mt-3">${p.name}</h5><div class="text-primary fw-bold">${formatPrice(p.price)}</div><a href="product.html?id=${p.id}" class="btn btn-sm btn-outline-primary mt-2">View</a></div>`;
    el.appendChild(col);
  });
}
...
function renderProductGrid(){
  ...
  sampleProducts.forEach(p=>{
    ...
    col.innerHTML = `<div class="card-e p-3 h-100"><div class="product-thumb">📁</div><h5 class="mt-3">${p.name}</h5><p class="text-muted small">${p.desc}</p><div class="d-flex justify-content-between align-items-center"><div class="fw-bold text-primary">${formatPrice(p.price)}</div><div><button class="btn btn-sm btn-primary" data-id="${p.id}">Add</button> <a class="btn btn-sm btn-outline-secondary" href="product.html?id=${p.id}">Details</a></div></div></div>`;
